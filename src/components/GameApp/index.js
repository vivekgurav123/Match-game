...
// const randomImageUrl = imagesList[randomNo].imageUrl

class GameAppss extends Component {
  state = {
  ...
  onImgClick = thumbnailUrl => {
    ...
    const imgVal = imagesList.filter(
      eachObj => eachObj.thumbnailUrl === thumbnailUrl,
    )
    // console.log(imgVal[0])
    const {imageUrl} = imgVal[0]
    if (imgurl === imageUrl) {
  ...
  playAgain = () => {
    ...
    this.timerId = setInterval(this.countDown, 1000)
    console.log('plY agein')
  }

  render() {
    const {score, imgurl, timer, isTrue} = this.state
    const results = this.getResultedList()
    return (
